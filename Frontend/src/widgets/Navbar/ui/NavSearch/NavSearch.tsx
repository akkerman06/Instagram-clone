...
import { getProfileUser, getSearchLoading, getSearchUsers, profileActions, searchUsers } from '@/entities/Profile'
...
export const NavSearch = () => {
  ...
  const onSearchUsers = useMemo(() => debounce(async(query) => {
    dispatch(searchUsers({search}))
  } , 1000) , [])
        ...
        <HStack className={cls.list} align='center' justify='center'>
          {
            ...
            </VStack>) : search && !searchLoading ? 
              <Text>Ничего не найдено</Text> : searchLoading && <Spin size='small' />
          }
        </HStack>
      }
